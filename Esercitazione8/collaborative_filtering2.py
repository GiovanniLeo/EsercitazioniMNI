...
def getFilmRating(person):
    ...
    for user in other:
        sim = similarity_score(person, user)
        if sim <= 0:
            continue
        flag = True
...
user = 'Toby'
userReck = user_reommendations(user)
for i in userReck:
    print("Film: {} ".format(i[1]))
    print("Stima: {0:.2f}".format(i[0]), "\n")

print("--  Indice di similarità con gli altri utenti rispetto a {} --".format(user))

userNumber = len(getOtherPeopleExceptPerson(user))
msu = most_similar_users(user, userNumber)

for i in msu:
    print("{:.2f}".format(i[0]), i[1])

print("--- Valutazione dei film ---")
getFilmRating(user)

print("---Indice similarità per valutazione dei film ---")
getFilmRatingSim(user)
